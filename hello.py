...
ReportDict = ["Nielsen", "Eyeota Branded", "Eyeota Whitelabel", "LiveRamp", "TTD", "Lotame"]
...
with open('hello.csv', 'rU') as csv_file:
	...
	if UsageReportInput == 'TTD':
		...
		fieldnames = ['Provider Name', 'Seller Name','Buyer Name','Behavior Id','Behavior Path','Revenue','Payout','Impressions','CPM','Type']
	elif UsageReportInput == 'Eyeota Branded':
		fieldnames = ['Platform', 'Buyer', 'Segment','Gross Revenue', "50%''rev share"]
	...
	with open('new_hello.csv', 'w') as new_file:
		...
		csv_writer = csv.DictWriter(new_file, fieldnames=newheader, delimiter=',')
...
#--------------------------------------TTD Report---------------------------------------
			if UsageReportInput == 'TTD':
				...
				if line['FullPath'][:firstarrow] == 'Custom Segment' or line['FullPath'][:firstarrow] == 'Interest from Social Activity' or line['FullPath'][:firstarrow] == 'Political' or line['FullPath'][:firstarrow] == 'Winter Holiday':
					...
					newInputdict['Country'] = line['FullPath'][:firstarrow]
...
#--------------------------------------Nielsen Report---------------------------------------
				...
				newInputdict['Region'] = line['Publisher']
...
#--------------------------------------Lotame Report---------------------------------------
				...
				newInputdict['Partner Name']= 'N/A'
				newInputdict['Advertiser Name']= 'N/A'
				newInputdict['Buyer Name']= line['Buyer Name']
...
#--------------------------------------Eyeota Branded Report---------------------------------------
				...
				newInputdict['Region'] = line['Segment'][:firstarrow]
				...
				newInputdict['Segment Name']= line['Segment']
				newInputdict['Revenue']= line['50%''rev share']
				newInputdict['Segment ID']= 'N/A'
...
#----------------------------Start Segment Type Categorization-------------------------------
			if UsageReportInput == 'Eyeota Whitelabel':
				...
				seasonalcheck = newInputdict['Segment Name'].find('Seasonal')
				...
				elif seasonalcheck > -1:
					if newInputdict['Segment Name'].find('Holidays & Seasonal Events') > -1:
						newInputdict['Segment Type'] = 'Vertical'
					else:
						newInputdict['Segment Type'] = 'Seasonal'
				elif lifecheck > -1:
					newInputdict['Segment Type'] = 'Life Event'
				elif customcheck1 > -1 or customcheck2 > -1 or customcheck3 > -1 or customKWcheck > -1:
					newInputdict['Segment Type'] = 'Custom'
